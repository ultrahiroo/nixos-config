...
# /boot/firmware partition, and {ext4 or btrfs} root partition. The generated image
...
let
  fsType = "btrfs";
  rootfsImage = (import "${inputs.nixpkgs}/nixos/lib/make-${fsType}-fs.nix") (
    {
...
in
{
  ...
  #     "The boot files for SD image have been moved to the main ${fsType} partition. The FAT partition now only holds the Raspberry Pi firmware files. Changing its size may not be required."
  #   )
