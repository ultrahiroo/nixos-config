...
keymap.set('<PageDown>', function() -- TODO: multibyte charactor support
  local move = page_movement_size()
  local _, lnum, col, off, _ = unpack(vim.fn.getcurpos('.'))
  local line_count = vim.api.nvim_buf_line_count(0)
  local new_lnum = math.min(lnum + move, line_count)
  vim.api.nvim_win_set_cursor(0, { new_lnum, col + off - 1 })
end, { 'n', 'i' })

keymap.set('<PageUp>', function() -- TODO: multibyte charactor support
  local move = -page_movement_size()
  local _, lnum, col, off, _ = unpack(vim.fn.getcurpos('.'))
  local new_lnum = math.max(lnum + move, 1)
  vim.api.nvim_win_set_cursor(0, { new_lnum, col + off - 1 })
end, { 'n', 'i' })
